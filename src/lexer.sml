structure Lexer :> JUNE_LEXER = struct
  ...
  fun takeWhile _ pos [] = ([], pos, [])
    | takeWhile predicate pos (c :: cs) =
        if predicate c then
          let
            val (taken, pos', rest) =
                takeWhile predicate (advance pos c) cs
          in
            (c :: taken, pos', rest)
          end
        else
  ...
  fun lexNumber pos chars =
    let
      val (digits, endPos, rest) =
        takeWhile Char.isDigit pos chars
    in
  ...
  fun lexSymbol pos chars =
    let
      val (name, endPos, rest) =
        takeWhile (not o isDelimiter) pos chars
  ...
  fun lexChars pos [] =
    ...
    | lexChars pos (chars as c :: rest) =
        if Char.isSpace c then
          ...
          let
            val (token, nextPos, rest') =
              lexNumber pos chars
          in
            token :: lexChars nextPos rest'
          end
        else
          let
            val (tok, nextPos, rest') =
              lexSymbol pos chars
          in
            tok :: lexChars nextPos rest'
          end
