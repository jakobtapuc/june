structure Evaluator :> JUNE_EVALUATOR = struct
  ...
  local
    open Ast
  in
  fun evaluateArgs (Value.Env env) [] =
        ([], Value.Env env)
      ...
      | Value.Closure { params, body, env, pos = pos' } =>
          let
            fun zip [] [] = []
              ...
              | zip _ _ = raise Value.Value ("Invalid number of arguments", pos')
  ...
  and evaluate (Value.Env env) expr =
    case expr of
      | Integer (n, _) =>
          (Value.Integer n, (Value.Env env))
      | Symbol ("#t", _) =>
          (Value.Boolean true, (Value.Env env))
      | Symbol ("#f", _) =>
          (Value.Boolean false, (Value.Env env))
      | Symbol ("#undef", _) =>
          (Value.Undef, (Value.Env env))
      | Symbol (s, pos) =>
          (lookup (Value.Env env) (s, pos), (Value.Env env))
      | List ([], pos) =>
      ...
      | List (Symbol ("if", pos) :: rest, _) =>
          (case rest of
            | [cond, onTrue, onFalse] =>
                let
                  val (condValue, (Value.Env env')) = evaluate (Value.Env env) cond
                in
                  (case condValue of
                    | Value.Boolean true =>
                        let
                          val (trueValue, (Value.Env env'')) = evaluate (Value.Env env') onTrue
                        in
                          (trueValue, (Value.Env env''))
                        end
                    | Value.Boolean false =>
                        let
                          val (falseValue, (Value.Env env'')) = evaluate (Value.Env env') onFalse
                        in
            ...
            | _ =>
                raise Value.Value ("Malformed `if`", pos))
      | List (Symbol ("define", pos) :: rest, _) =>
          (case rest of
            ...
            | _ =>
                raise Value.Value ("Malformed `define`", pos))
      | List (f :: args, pos) =>
          let
    ...
    and evaluateSeq _ env [expr] =
          evaluate env expr
      | evaluateSeq pos env (expr :: rest) =
          let
            val (_, env') = evaluate env expr
          in
            evaluateSeq pos env' rest
          end
      | evaluateSeq pos _ [] =
