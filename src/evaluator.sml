...
struct
  ...
  and apply fnValue args pos =
    ...
    | VClosure {params, body, env, pos = pos', ...} =>
        let
          val frame = {name = NONE, def = SOME pos', call = SOME pos}

          val () = StackTrace.push frame

          val bound = ListExt.zip params args
                      handle | _ => failWithTrace "Argument count mismatch" pos
          ...
          val (result, _) = evaluateSeq pos' env' body
                            handle exn => (StackTrace.pop (); raise exn)

          val () = StackTrace.pop ()
        in
          result
        end
    | _ => failWithTrace "Attempt to call a non-function value" pos
  ...
  and evaluate (Env env) expr =
    ...
    | List ([], pos) => failWithTrace "Cannot evaluate an empty list" pos
    | List (Symbol ("quote", _) :: [quoted], _) => (quoteValue quoted, Env env)
    | List (Symbol ("lambda", pos) :: List (params, _) :: body, _) =>
        let
          fun paramName (Symbol (p, _)) = p
            | paramName _ =
                failWithTrace "Param name has to be a symbol" pos

          val paramNames = List.map paramName params
        in
          if null body then
            failWithTrace "Lambda needs a body" pos
          else
            ( VClosure
    ...
    | List (Symbol ("if", pos) :: rest, _) =>
        (case rest of
         ...
         | _ => failWithTrace "Malformed `if`" pos)
    | List (Symbol ("define", pos) :: rest, _) =>
        (case rest of
         ...
         | _ => failWithTrace "Malformed `define`" pos)
    | List (Symbol ("set!", pos) :: rest, _) =>
        (case rest of
         ...
         | _ => failWithTrace "Malformed `set!`" pos)
    | List (Symbol ("lazy", pos) :: [thunk], _) =>
        ( VPromise
            { objectId = ObjectId.newId ()
            , body = thunk
            , env = Env env
    ...
    | List (Symbol ("force", pos') :: [name], _) =>
        let
          val (promise, env') = evaluate (Env env) name
          fun force (VPromise {body, env, value, ...}) =
                (case !value of
                 | SOME result => result
                 | NONE =>
                     let
                       val (result, _) = evaluate env body
                       val () = value := SOME result
                     in
                       result
                     end)
            | force _ =
                failWithTrace "Cannot evaluate a non-promise object" pos'
        in
          (force promise, env')
        end
    | List (Symbol ("begin", pos) :: exprs, _) =>
        evaluateSeq pos (Env env) exprs
    | List (f :: args, pos) =>
        let
